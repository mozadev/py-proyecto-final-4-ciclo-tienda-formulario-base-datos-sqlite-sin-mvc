...
class Index(Frame):
	def __init__(self,raiz):
		...
		x_ventana = self.raiz.winfo_screenwidth() // 2 - ancho_ventana // 2 # para redimensinar el tamaño // doble division saca entero
		y_ventana = self.raiz.winfo_screenheight() // 2 - alto_ventana // 2

		posicion = str(ancho_ventana) + "x" + str(alto_ventana) + "+" + str(x_ventana) + "+" + str(y_ventana)
		self.raiz.geometry(posicion)  # se pasa con variables el geometry
		...
		self.pack() # empaqueta para q aparesca uno debajo de otro
		self.widgets()
		self.place(relwidth=1, relheight=1)# la venta se acople al cambio , la ventana se acpla a los cambios
